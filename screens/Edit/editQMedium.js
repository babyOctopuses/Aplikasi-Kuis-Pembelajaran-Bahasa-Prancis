...
class EditQMedium extends Component {
  ...
  inputValueUpdateVocab = (val, index, prop) => {
    const state = this.state.Medium[0];
  ...
  deleteQuestions(index){
    if(this.state.Medium[0].length==1){
      Alert.alert(
        'Delete User',
        'There must be at least 1 Question',
        [{text: 'Got it!'}]
      )
    }else if(this.state.Medium[0].length>1){  
    ...
    updateDBRef.set({
      title: this.state.title,
      desc: this.state.desc,
      Medium:this.state.Medium[0].filter(quizes => quizes.Question != this.state.Medium[0][index].Question),
      Easy:[...this.state.Easy[0]],
      Hard:[...this.state.Hard[0]],
    }).then((docRef) => {
      this.setState({
