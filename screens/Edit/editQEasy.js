...
class EditQEasy extends Component {
  ...
  deleteQuestions(index){
    if(this.state.Easy[0].length==1){
      Alert.alert(
        'Delete Question',
        'There must be at least 1 Question',
        [{text: 'Got it!'}]
      )
    }else if(this.state.Easy[0].length>1){  
    ...
    updateDBRef.set({
      ...
      number: this.state.number,
      lesson: this.state.lesson,
      Easy:this.state.Easy[0].filter(quizes => quizes.Question != this.state.Easy[0][index].Question),
      Medium:[...this.state.Medium[0]],
      Hard:[...this.state.Hard[0]],
    }).then((docRef) => {
      this.setState({
